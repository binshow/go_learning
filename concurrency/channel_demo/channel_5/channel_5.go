...
// buffered channel_demo 的使用案例一
// Fan Out Pattern（扇出模式):https://zhuanlan.zhihu.com/p/410523401
// 扇出是指多个函数同时从相同的channel中读取数据直到channel关闭
...
// The buffered channel_demo is fantastic here because we know ahead of time that there are 10
...
func main() {
	...
	// Buffered channel_demo to receive information about any possible insert.
	...
	// 开启 10个 goroutine 并行的去往数据库插入数据，插入结果放入到channel中去
	for i := 0; i < routines; i++ {
		go func(id int) {
			// 模拟数据库
			ch <- insertUser(id)

			// We don't need to wait to start the second insert thanks to the buffered channel_demo.
